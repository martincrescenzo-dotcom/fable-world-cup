...
**★ TIER-1 RED-TEAM RUN (3 agents, REVIEW_2026-06-29): PICK STANDS — all numbers reproduce exactly, no FATAL/pick-change. Two rationale over-claims FIXED (both pick-neutral):** (1) "+2.7 EV-max" overstated → it's a NEAR-EV-TIE shaded to NL (σ-significance test is near-circular when model≈market; gap leans on the chalk-biased wshare split). Morocco is a genuine close alt, rejected for being outside the 5% DIFF band (−2.7 EV / 6.4% below). (2) **STRUCK "field under-backs NL" — OUTPUT-GATE violation: 39% is the PLURALITY field pick → NL is correlated with the pack, not decorrelated; the 3pp vs house line is noise.** Blend-then-transform shortcut verified = identical to proper transform here (model≈market). G1 correctly off (long horizon); re-eval at QF (N~7). See prediction.md R32 block + REVIEW_2026-06-29.

### ─── SESSION LOG 2026-06-29 (reflections · questions · conclusions; prepared for clear) ───
**WHAT HAPPENED:** user reported SA-Canada result (0-1, +50) + handed the next match (NL-Morocco, rewards 80/114/122, field 39/30/31). Made the NL pick, then user asked (a) "were 90'/120' the same or did it need a tweak?" and (b) "does a red-team validate the full approach?" → ran a Tier-1 3-agent red-team on the pick.
**REFLECTIONS / CONCLUSIONS:**
 1. **ET transform is automated & load-bearing for DRAWS, near-decorative for the OUTCOME on clear-fav matches.** 90'→120' did NOT flip NL or the modal (1-0) for NL-Morocco; it crushed Draw EV 31.8→20.1 (group draw-harvesting does not transfer, as doctrine says). Across the 8 live R32 matches in ko_build it flips 0 outcome picks but re-prices every draw down. φ=0.635 robust: pick & +2.7 margin invariant across CI [0.44,0.88].
 2. **The pick is right; the RATIONALE was over-decorated — two self-inflicted "noise-as-signal" slips the gate is meant to catch.** (i) σ_EV significance test is near-CIRCULAR when model≈market → don't call a reward-asymmetry EV gap "significant"; it's a near-EV-tie shaded to the favourite. (ii) a PLURALITY field pick (39%) is CORRELATED with the pack, not "underpicked/decorrelated." Both struck. New memory: [[ev-tie-and-plurality-correlation]].
 3. **Red-team value confirmed even on a routine follow:** all 9 numbers reproduced exactly (no arithmetic error), so the catch was purely at the inference/framing layer — the recurring project failure mode. "Numbers right, inference wrong" again.
**OPEN QUESTIONS / NEXT ACTIONS (carry forward):**
 - **OTHER R32 PICKS NOT YET SUBMITTED:** ko_build has Mexico-Ecuador, Belgium-Senegal, Portugal-Croatia, Switzerland-Algeria, Colombia-Ghana, Australia-Egypt (model-only ran). Each still needs the full procedure (independent market pull + model-blind scan + blend120) before submitting. Brazil 1-0 Japan & Germany 1-0 Paraguay already submitted (2026-06-28) — PENDING results/tiers.
 - **RESIDUAL on NL-Morocco:** confirm the de-vig ML line (+115/+230/+260) post-dates Morocco's lineup confirmation; if it predates → small unpriced nudge toward Morocco (tightens the tie, does NOT flip). Optional fresh pull pre-kickoff.
 - **G1 endgame:** re-evaluate the field-proxy switch at the QF stage (N≈7), where horizon condition can flip. Still OFF now.
 - **OPEN Q FOR USER (unanswered):** is ANY rival pick info ever visible (post-lock reveal / shared sheet / top-players CSV)? If never, G1 stays the blunt field-proxy permanently.
 - **wshare supremacy split** (freed-draw → favourite) remains unvalidated/chalk-biased (567dfbb) — a deploy caveat, harmless when model≈market, watch on divergent KO matches.
 - **NEED FROM USER each KO match:** rewards, field %, realized tier of the actual 120' score, updated leaderboard.
...
**MD6 results (3/4 outcomes, TWO EXACT SCORES):**
 - **Scotland 0–1 Morocco → HIT + EXACT** (pick Morocco 0-1) → base 91 + **bonus +50** = 141. obs#27.
 - **Brazil 3–0 Haiti → HIT + EXACT** (pick Brazil 3-0) → base 21 + **bonus +20** = 41. obs#28.
 - **USA 2–0 Australia → HIT** (pick USA 1-0; **OVERRIDE of engine's Australia VINDICATED** — USA-underrating artifact
   resolved FOR market AGAIN, 3rd time incl USA-Paraguay 4-1) → base 58, score miss. 2-0 tier **+30** obs#30.
 - **Türkiye 0–1 Paraguay → MISS** (pick Turkey 1-0 follow; Paraguay won away, both must-win) → base 0. 0-1 tier +20 obs#29.
 Reconciles EXACTLY: 141+41+58 = +240. **The USA artifact call is now 3/3 FOR the market — high confidence in the veto.**
**LEADERBOARD 2026-06-20 (16):**
 #1 Alexlastaaaaar 1594 (19/4) | #2 Hadri02 1307 (18/**7**) | #3 Nicolas232345 1271 (19/2) | #4 AdyFC 1248 (18/**7**) |
 #5 Alexis#KGYJ 1232 (19/4) | #6 CrazyBE 1208 (19/2) | #7 Cyrilpqt 1190 (16/3) | #8 Chocho_27 984 (16/1) |
 **#9 USER (Lampadaire83) 937 (14/3)** | #10 Paulinho1204 919 (15/2) | #11 Diane123 905 (11/3) | #12 Ethan_prn 878 (16/2) |
 #13 LMD92110 681 | #14 Helene12ln 663 | #15 DealNonClosey 628 | #16 FredFUP 587.
**STRATEGIC READ:** top-5 line = #5 Alexis **1232, gap 295**. Cluster #2–#7 bunched (1190–1307, 117 over 6) → catchable WITH
a hot exact/contrarian run; #8 only +47. **The climbers all have HIGH EXACTS** (#2 Hadri & #4 AdyFC = 7 exacts each; both
jumped). Exacts + field-underpicked contrarian outcomes = the lever, confirmed again. User now has the ammo (3 exacts, +240).
...
| 2026-06-19 | Scotland 0–1 Morocco | Morocco W | **HIT + EXACT** — pick Morocco 0-1 → base 91 + **bonus +50** | 0-1 tier **+50** obs#27 |
